...
const PROMPTS = {
  headlines: () =>
    `Current date/time (UTC): ${new Date().toISOString()}. Use this to judge how recent a story is and to fill in published_at accurately.
...
For every story, find its actual publish date/time from the article itself or the search result metadata (bylines, datelines, and search snippets usually include this — e.g. "3 hours ago", "July 12, 2026"). Convert whatever you find into a real ISO 8601 UTC timestamp for published_at. If only a date is available with no time, use 12:00 UTC that day. Never leave published_at as your best guess if the source doesn't support it — in that case set it to null instead of fabricating a time.
` +
    wrapInstruction(
      `{"main_headline":{"title":"string","dek":"2-sentence summary","source":"outlet","url":"real URL","category":"Israel Politics | Israel Economy | World Geopolitics | World Economy","published_at":"ISO 8601 UTC timestamp or null"},"world_stories":[{"title":"string","one_liner":"1 sentence","source":"outlet","url":"real URL","category":"short label","published_at":"ISO 8601 UTC timestamp or null"}],"stories":[{"title":"string","one_liner":"1 sentence","source":"outlet","url":"real URL","category":"short label","published_at":"ISO 8601 UTC timestamp or null"}]}
...
Ticker accuracy matters a lot here — do not guess or infer a ticker from the company name. Only use the exact ticker symbol as it literally appears in the source article or in a search result for "<company name> stock ticker". Include which exchange it trades on (e.g. NASDAQ, NYSE, TASE, LSE) alongside the ticker, since many companies are cross-listed and the same name can map to different symbols on different exchanges. If you cannot confirm the exact ticker from a search result, say so honestly in the ticker field (e.g. "unconfirmed") rather than fabricate one.
` +
    wrapInstruction(
      `{"ticker":"exact symbol as found, e.g. NVDA","exchange":"e.g. NASDAQ","company":"name","magazine_source":"publication","headline":"article's angle","summary":"3-4 sentence analysis: why it's news, bull/bear case","url":"real URL"}`
    ),
  podcast: () =>
